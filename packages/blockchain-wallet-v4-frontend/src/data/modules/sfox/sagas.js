...
export default ({ coreSagas }) => {
  ...
  const getJumio = function*() {
    try {
      yield put(A.sfoxLoading())
      const jumioStatus = yield call(
        coreSagas.data.sfox.getEnhancedVerificationStatus
      )
  ...
  const initializeJumio = function*() {
    try {
      yield put(A.sfoxLoading())
      const jumioInitialized = yield call(
        coreSagas.data.sfox.startEnhancedVerification
      )
